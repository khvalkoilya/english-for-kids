...
export function hidingMenu(e) {
    if (vars.sideMenu.classList.contains('side-menu_active') && (!e.target.classList.contains('burger') && !e.target.parentElement.classList.contains('burger'))) {
...
export function changeActiveLink(elem) {
    ...
    if (vars.currentPage === 'Main Page') main.mainPageBlock();
    else if (vars.currentPage === 'Statistics') createHugeArray();
    else topic.findPositionOfTopic(vars.currentPage);
}